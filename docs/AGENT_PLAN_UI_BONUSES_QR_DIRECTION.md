...
- Ordinary proximity requires both positions to be no more than two minutes old. Preserve the five bands: up to 25 m, 100 m, 300 m, 1 km, and over 1 km. Preserve displayed distance rounding to 10 m.
...
| G4 | What does “exact coordinates” mean? | One timestamped GPS snapshot of the current target is narrower than live tracking. Choose snapshot versus time-limited live access, display lifetime, and what happens when a target changes. GPS accuracy is not physical certainty. | Unresolved |
